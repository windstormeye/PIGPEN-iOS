...
class PJPetDrinkDetailsView: UIView {
    ...
    var viewModel = ViewModel() {
        didSet {
            ...
            detailsViewModel.firstString = "\(viewModel.drink.water_target_today) ml"
            detailsViewModel.secondString = String(viewModel.drink.times)
            detailsViewModel.thirdString = "\(viewModel.drink.water_today) ml"
            detailsViewModel.score = CGFloat(viewModel.drink.score)
    ...
    private func initView() {
        ...
        manualAddKcalView = PJManualAddKcalView(frame: CGRect(x: 0, y: addButton.top - 20, width: pj_width, height: 60), pickCount: 2000, tipsText: "ml")
...
extension PJPetDrinkDetailsView {
    @objc
...
private extension Selector {
    static let edit = #selector(PJPetDrinkDetailsView.edit)
    static let add = #selector(PJPetDrinkDetailsView.add)
}

extension PJPetDrinkDetailsView {
    struct ViewModel {
        var drink: PJPet.PetDrink
        var pet: PJPet.Pet
        
        init() {
            drink = PJPet.PetDrink()
            pet = PJPet.Pet()
        }
    }
}
